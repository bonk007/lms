<x-layouts.blank>
    <x-slot:title>{{ __("Quiz Preface") }}</x-slot:title>

    <div class="py-8">
        <div class="text-2xl font-semibold">{{ __("Courses") }}</div>
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('courses.show', compact('course')) }}" class="hover:text-red-600">{{ $course->title }}</a>
            <div class="text-slate-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                </svg>
            ...
            <a href="#" disabled class="text-slate-600">{{ $quizdata['title'] }}</a>
        </div>
    </div>
