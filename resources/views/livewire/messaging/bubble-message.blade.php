<div wire:init="read">
    <div class="flex items-start gap-4 px-4 py-2 {{ $isOwned ? 'justify-end' : '' }}">
        @if(!$isOwned)
            <x-chunks.avatar name="{{$message->getAttribute('sender')?->getAttribute('name')}}" online="{{ $message->getAttribute('sender')?->getAttribute('is_online') ? 'true' : 'false' }}" />
        @endif
        <div class="flex flex-col w-1/2 px-4 py-2.5 border rounded-xl {{ !$isOwned ? 'rounded-tl-none' : 'rounded-tr-none' }}">
            ...
            <div class="flex justify-end items-center gap-2">
                <span class="text-xs">{{ $message->getAttribute('created_at')->diffForHumans(now()) }}</span>
                @if($isOwned)
                <span class="{{ $message->getAttribute('is_read') ? 'text-blue-500' : '' }}">
